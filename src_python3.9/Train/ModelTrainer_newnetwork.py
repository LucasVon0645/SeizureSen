...
class ModelTrainer:
    ...
    def train(self):
        ...
        multi_view_conv_model, early_stopping = MultiViewConvModelWithAttention.get_model(config)
        ...
        if len(y_train.shape) != 2:
            y_train = y_train.reshape(-1, 2)  # 根据需要调整维度
        if len(y_val.shape) != 2:
            y_val = y_val.reshape(-1, 2)

        print("Adjusted y_train shape:", y_train.shape)
        print("Adjusted y_val shape:", y_val.shape)

        y_train = y_train.astype("float32")
        y_val = y_val.astype("float32")
        multi_view_conv_model.summary()
        multi_view_conv_model.compile(
            ...
            loss_weights={
                "final_output": 1.0,  # 主输出的损失权重
                "early_exit1": 0.5,  # 辅助输出的损失权重
                "early_exit2": 0.7,
                "early_exit3": 0.8,
                "early_exit4": 1.0,
            },
            metrics={
                "final_output": ["accuracy"],  # 每个输出的评估指标
        ...
        # 训练模型
        history = multi_view_conv_model.fit(
            {"time_domain_input": X_pca_train, "freq_domain_input": X_fft_train},  # 输入数据
            {
            ...
            validation_data=(
                {"time_domain_input": X_pca_val, "freq_domain_input": X_fft_val},  # 验证输入
                {
                    "final_output": y_val,
                    "early_exit1": y_val,  #
                    "early_exit2": y_val,
            ...
            epochs=config["nb_epoch"],  # 训练的 epoch 数
            batch_size=config["batch_size"],  # 每批次的大小
            callbacks=[early_stopping, checkpoint],  # 回调函数
        )

        print([output.name for output in multi_view_conv_model.outputs])
        ...
        print("\nValidation Accuracy for 'final_output' over epochs: ", history.history["val_final_output_accuracy"])
        print("Validation Accuracy for 'early_exit1' over epochs: ", history.history["val_early_exit1_accuracy"])
        print("Validation Accuracy for 'early_exit2' over epochs: ", history.history["val_early_exit2_accuracy"])
        print("Validation Accuracy for 'early_exit3' over epochs: ", history.history["val_early_exit3_accuracy"])
        print("Validation Accuracy for 'early_exit4' over epochs: ", history.history["val_early_exit4_accuracy"])
    ...
    def _get_classification_report(self, y_true, y_pred, suffix=""):
        ...
        y_pred_final = np.array(y_pred[0])
